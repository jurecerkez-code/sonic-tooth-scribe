...
export const VoiceRecording = ({
  ...
  const sendAudioToN8n = async (audioBlob: Blob, retryAttempt = 0): Promise<boolean> => {
    ...
    try {
      ...
      const extension = mimeType.includes("mp4") ? "mp4" : mimeType.includes("mpeg") ? "mp3" : mimeType.includes("webm") ? "webm" : "webm";
      if (retryAttempt > 0) {
        toast({
      ...
      toast({
        ...
        description: retryAttempt > 0 ? "Recording uploaded successfully!" : "Voice recording sent to automation workflow"
      });
      ...
      console.error("n8n Error (exception):", {
        error,
        attempt: retryAttempt + 1
      });
      if (retryAttempt < MAX_RETRY_ATTEMPTS - 1) {
  ...
  const retryFailedRecordings = async () => {
    ...
    if (remaining.length === 0) {
      toast({
        ...
        description: "All offline recordings synced!"
      });
      setOfflineMode(false);
    }
  };
  ...
  return <div className="bg-card border border-border rounded-lg p-8 shadow-sm">
      <div className="flex flex-col items-center space-y-6">
        {/* Status */}
        <div className="w-full flex items-center justify-between">
          <Badge variant={n8nStatus === "healthy" ? "default" : n8nStatus === "slow" ? "secondary" : "destructive"} className="flex items-center gap-2">
        ...
        {/* Offline banner */}
        {offlineMode && <Alert variant="destructive" className="w-full">
        ...
        <div className="relative">
          {isRecording ? <Button size="lg" variant="destructive" onClick={stopRecording} className="w-32 h-32 rounded-full text-lg font-semibold shadow-lg">
              <div className="flex flex-col items-center gap-2">
                <Square className="w-8 h-8" />
                <span>Stop</span>
              </div>
            </Button> : <Button size="lg" onClick={startRecording} disabled={isProcessing} className="w-32 h-32 rounded-full text-lg font-semibold shadow-lg bg-primary hover:bg-primary/90">
              {isProcessing ? <Loader2 className="w-8 h-8 animate-spin" /> : <div className="flex flex-col items-center gap-2">
                  <Mic className="w-8 h-8" />
                  <span>Record</span>
                </div>}
            </Button>}

          {isRecording && <div className="absolute inset-0 -m-4 pointer-events-none">
              <div className="w-full h-full rounded-full bg-recording-glow animate-pulse-recording" />
            </div>}
        </div>

        {/* Waveform */}
        {isRecording && <div className="w-full max-w-md space-y-4">
            <canvas ref={canvasRef} width={600} height={100} className="w-full h-24 bg-card rounded-lg border border-border" />
        ...
        {/* Instructions */}
        {!isRecording && !isProcessing && !transcript && <p className="text-muted-foreground text-center max-w-md">
        ...
        {transcript && <div className="w-full space-y-4">
            <div className="bg-accent rounded-lg p-4">
              <h3 className="text-sm font-semibold text-accent-foreground mb-2">Your Recording:</h3>
              <p className="text-foreground">{transcript}</p>
            </div>

            {aiResponse && <div className="bg-secondary/10 border border-secondary rounded-lg p-4">
